  ...
  export default function Category() {
    ...
    useEffect(() => {
      async function fetchListings() {
        try {
          ...
          toast.error(`could not fetch ${param.categoryName}`);
        }
      }
      fetchListings();
    }, [param.categoryName]);
    ...
    return (
      <div className="max-w-6xl mx-auto px-3">
        <h1 className="text-3xl text-center mt-6  font-bold mb-6 ">{`Place for ${param.categoryName}`}</h1>
        {loading ? (
          <Spinner />
          ...
          <p>there are no current <span className="uppercase">{param.categoryName}</span></p>
        )}
      </div>
    );
  }
